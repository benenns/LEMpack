...
ode.list.OAT = function (x, x.offoat, beta_o, beta_s, gamma, rho, ws, wp, mu_mat, mo, eta,
...
dS1 = rho*sum(x) - psi*S1 + ws*S2 - lambda*S1               - mo[1]*S1  + oat.e*x.offoat[1] - oat.q*S1 - rho.m*sum(x[4:19]) - eta*S1
dS2 = psi*S1 + wp*Sp      - ws*S2 - lambda*S2               - mo[2]*S2  + oat.e*x.offoat[2] - oat.q*S2 - eta*(S2)
dSp =        - wp*Sp              - lambda.p*Sp             - mo[3]*Sp  + oat.e*x.offoat[3] - oat.q*Sp + eta*(S2+S1)

dIa = lambda*(S1+S2) - psi*Ia      - theta.ai*Ia            - mo[4]*Ia  + oat.e*x.offoat[4] - oat.q*Ia  + rho.m*Ia
...
dIap= lambda.p*Sp    - psi.p*Iap   - theta.ai*Iap           - mo[8]*Iap + oat.e*x.offoat[8] - oat.q*Iap + rho.m*Iap
dIp = theta.ai*Iap   - psi.p*Ip                 - mu_mat*Ip - mo[9]*Ip  + oat.e*x.offoat[9] - oat.q*Ip  + rho.m*Ip

dDa =               psi*Ia            + psi.p*Iap - theta.ad*Da                        - mo[10]*Da + oat.e*x.offoat[10] - oat.q*Da + rho.m*Da
dD1 = theta.ad*Da + psi*(1-phi[1])*I1 + psi.p*Ip  - theta.1*D1 - alpha1*D1 - mu_mat*D1 - mo[11]*D1 + oat.e*x.offoat[11] - oat.q*D1 + rho.m*D1
dD2 = theta.1*D1  + (psi+v2)*(1-phi[2])*I2        - theta.2*D2 - alpha2*D2 - mu_mat*D2 - mo[12]*D2 + oat.e*x.offoat[12] - oat.q*D2 + rho.m*D2
dD3 = theta.2*D2  + (psi+v3)*(1-phi[3])*I3                     - alpha3*D3 - mu_mat*D3 - mo[13]*D3 + oat.e*x.offoat[13] - oat.q*D3 + rho.m*D3
...
dO1 = theta.t1O*T1 - theta.1*O1   - alpha.re1*O1           - mu_mat*O1 - mo[17]*O1 + oat.e*x.offoat[17] - oat.q*O1 + rho.m*O1
dO2 = theta.t2O*T2 + theta.1*O1   - (alpha.re2+theta.2)*O2 - mu_mat*O2 - mo[18]*O2 + oat.e*x.offoat[18] - oat.q*O2 + rho.m*O2
