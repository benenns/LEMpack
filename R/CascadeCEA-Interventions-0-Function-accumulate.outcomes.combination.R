...
accum.outcomes.combination<- function(input.parameters,
  ...
  #########!!!!! only for PrEP expansion analysis
  tf.temp[TRUE] <- FALSE
  ...
  # Hold parameters for possible COVID-19 analysis
  params.hold <- input.parameters[c("prep.total", "v.ssp", "nOAT.m",
                                    "psi.m", "alpha1", "alpha2", "alpha3",
                                    "T1_O", "T2_O", "T3_O", "O_T", "d","noG","nsG")]
  for(i in 1:(dim(pop.in)[1])){
    ...
    if(i > int.start & i <= (int.start + (20 * 12))){
      if (exists("COVID", where=input.parameters)){
        if (input.parameters$COVID == TRUE){
          if ((i >= 99) & (i <= 110)){
            #Modification of PrEP
            input.parameters$prep.total <- input.parameters$prep.total  - input.parameters$param.reduction.rate * input.parameters$prep.total * input.parameters$scenario["PrEP"]

            #Modification of SSP
            input.parameters$v.ssp <- input.parameters$v.ssp - input.parameters$param.reduction.rate * input.parameters$v.ssp * input.parameters$scenario["SSP"]

            #Modification of MOUD
            input.parameters$nOAT.m[4,] <- input.parameters$nOAT.m[4,]  - input.parameters$param.reduction.rate * input.parameters$nOAT.m[4,] * input.parameters$scenario["MOUD"]

            #Modification of HIV testing
            input.parameters$psi.m[4,] <- input.parameters$psi.m[4,] - input.parameters$param.reduction.rate * input.parameters$psi.m[4,] * input.parameters$scenario["HIV testing"]

            #Modification of ART parameters
            input.parameters$alpha1 <- input.parameters$alpha1 - input.parameters$param.reduction.rate * input.parameters$alpha1 * input.parameters$scenario["ART"]
            input.parameters$alpha2 <- input.parameters$alpha2 - input.parameters$param.reduction.rate * input.parameters$alpha2 * input.parameters$scenario["ART"]
            input.parameters$alpha3 <- input.parameters$alpha3 - input.parameters$param.reduction.rate * input.parameters$alpha3 * input.parameters$scenario["ART"]

            input.parameters$T1_O <- input.parameters$T1_O + input.parameters$param.reduction.rate * input.parameters$T1_O * input.parameters$scenario["ART"]
            input.parameters$T2_O <- input.parameters$T2_O + input.parameters$param.reduction.rate * input.parameters$T2_O * input.parameters$scenario["ART"]
            input.parameters$T3_O <- input.parameters$T3_O + input.parameters$param.reduction.rate * input.parameters$T3_O * input.parameters$scenario["ART"]

            input.parameters$O_T <- input.parameters$O_T - input.parameters$param.reduction.rate * input.parameters$O_T * input.parameters$scenario["ART"]

            #Modification of Behaviour
            input.parameters$d <- input.parameters$d - input.parameters$behaviour.change * input.parameters$d
            input.parameters$noG <- input.parameters$noG - input.parameters$behaviour.change * input.parameters$noG
            input.parameters$nsG <- input.parameters$nsG - input.parameters$behaviour.change * input.parameters$nsG
            }
            else{
              input.parameters[c("prep.total", "v.ssp", "nOAT.m",
                                 "psi.m", "alpha1", "alpha2", "alpha3",
                                 "T1_O", "T2_O", "T3_O", "O_T","d","noG","nsG")] <- params.hold
            }
        }
      }
      # QALYs
      ...
      psi.matrix.hold <- psi.matrix
      ## INTERVENTION
      if(is.element(c("Large-scale testing during COVID"),current.int)){
        if(i >= 102 & i <= 110){
        psi.matrix.temp <- matrix(0, nrow = 42, ncol = 19)
        colnames(psi.matrix.temp) <- state.name[-c(20:24)]
        if (input.parameters$testing.level.reach==0.25) {psi.increase <- 0.0180}
        if (input.parameters$testing.level.reach==0.5) {psi.increase <- 0.04}
        if (input.parameters$testing.level.reach==0.75) {psi.increase <- 0.0682}
        if (input.parameters$testing.level.reach==0.9) {psi.increase <- 0.0899}
        psi.matrix.temp[ , c("S1", "Ia", "I1")] <- psi + rep(psi.increase, 42)
        psi.matrix.temp[ , "I2"] <- psi + rep(psi.increase, 42) + input.parameters$v2
        psi.matrix.temp[ , "I3"] <- psi + rep(psi.increase, 42) + input.parameters$v3
        psi.matrix.temp<- t(psi.matrix.temp)
        psi.matrix <- psi.matrix.temp
        }else{
          psi.matrix <- psi.matrix.hold
        }
      }
    ...
    if(tf.temp$intervention.testing.modif      == FALSE &
       tf.temp$intervention.initiation.modif   == FALSE &
       tf.temp$intervention.retention.modif    == FALSE &
       tf.temp$intervention.dropout.modif      == FALSE &
       tf.temp$intervention.immediateART.modif == FALSE &
       tf.temp$intervention.reinitiation.modif == FALSE &
       tf.temp$intervention.ssp.modif          == FALSE &
       tf.temp$intervention.oat.modif          == FALSE &
       tf.temp$intervention.prep.modif         == FALSE &
       is.element(c("Large-scale testing during COVID"),current.int) == FALSE){
      # INTERVENTION COSTS
      int.costs.period[[i]]      <- (CurrentPeriod * 0)
      int.impl.costs.period[[i]] <- (CurrentPeriod * 0)
      int.sust.costs.period[[i]] <- (CurrentPeriod * 0)
