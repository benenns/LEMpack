#' Load city interventions workspace
#'
#' @param XX
#' @param
#'
#' @return
#' This module Sets global parameters, get inputs and initialize model
#' @export

source("ModelCoreModules/CascadeCEA-Model-1-Module-Data.input.R")
...
if (case == "DM"){     #For the case of deterministic analysis based on the mean value of calibrationed parameters
  ...
  if(file.exists(paste0("Inputs/AllParams-Combination-DM-", CITY, ".rds"))){
    ...
    source ("ModelCoreModules/CascadeCEA-Model-0-Parameter.update.R")  # Update free parameters with the mean calibrated values
