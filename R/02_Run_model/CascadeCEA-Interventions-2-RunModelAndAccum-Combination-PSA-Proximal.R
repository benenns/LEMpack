#' Run PSA for proximal strategies to OCIS
#'
...
param.sets   <- 2000
#Load proximal stratgies, if unavailable, execute: "CascadeCEA-Interventions-3-Analysis-DetermineProximalStrategies.R"
...
for (ss in 1:length(proximal.ind)){
  current.int = interventions[combination.list[[proximal.ind[ss]]]]
  ...
  saveRDS(outcome.comb.SA.mx,   paste0("Combination/Outcome-Combination-", CITY, "-PSA-", proximal.ind[ss],"(Proximal).rds"))
}
